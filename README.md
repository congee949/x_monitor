...
**AI**
- 双后端：`mimo`（推广识别 + 文章摘要，主）、`gemini`（视觉/兜底）
- 关键词初筛 + AI 复核的两级推广过滤，降低 AI 调用量

**可靠性与可观测**
- 账号连续失败达阈值（默认 4 轮 ≈ 2 小时）发一次 Telegram 告警并**置顶**；恢复后原地改写为「已恢复」并取消置顶
- 文章处理失败发通知，重试成功后原地改写为成功，不留悬空故障消息
- **置顶状态看板**：一条置顶消息每轮原地编辑刷新（编辑不触发通知，零打扰），显示上轮时间/耗时、当日推送计数、账号健康、异常明细；当日计数以北京时间每天 06:00 为界
- 看板**自适应规避聊天自动删除**：读取该聊天的 auto-delete TTL，在消息将近到期（TTL 的 85%）时主动重建并重新置顶
